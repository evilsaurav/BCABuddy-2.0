...
def chat_endpoint(request: Request, payload: ChatRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        ...
        try:
            ...
            import traceback
            import logging
            logging.error(f"AI Response Error: {traceback.format_exc()}")
        ...
        try:
            ...
            import traceback
            import logging
            logging.error(f"Payload Build Error: {traceback.format_exc()}")
            raise HTTPException(status_code=500, detail="Something went wrong. Please try again later.")
    except HTTPException:
        raise
    except Exception as e:
        import traceback
        import logging
        logging.error(f"Global Chat Error: {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail="Something went wrong. Please try again later.")
